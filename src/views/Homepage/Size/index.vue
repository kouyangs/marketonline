<template>
  <div class="container">
    ...
    <table class="table">
      ...
      <tbody>
        <tr v-for="(item, index) in size" :key="index">
          <td>{{index+1}}</td>
          <td>{{item.name}}</td>
          <td>
    ...
    <ModalEdit @close="closeModal()" :active="setState.edit">
      <EditItem @close="closeModal()" :lists="findSize" />
    </ModalEdit>
...
export default {
  ...
  methods: {
    ...
    OpenModalEdit(id) {
      this.findSize = { ...this.FilterSize(id) };
    ...
    FilterSize(id) {
      return this.size.filter(item => item.id == id)[0] || {}; // ຕ້ອງເເມ່ນ ==
    },
